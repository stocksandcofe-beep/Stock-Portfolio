...
// =============================================================================
// 4. FINANCIALS
// Primary source: Finnhub. Yahoo Finance is used as a fallback for any fields
// that Finnhub returns as null (common on the free tier for dividends, PE etc.)
// =============================================================================
async function fetchFinancials(symbol) {
    ...
    try {
        ...
        console.warn('Finnhub financials failed:', e);
    }

    // --- Yahoo Finance fallback ---
    // Fetched in parallel and used to fill any gaps Finnhub leaves behind.
    let yf = null;
    try {
        const res  = await fetch(`https://query1.finance.yahoo.com/v8/finance/chart/${symbol}?modules=summaryDetail,defaultKeyStatistics,financialData`);
        const data = await res.json();
        const modules = data?.chart?.result?.[0]?.meta || {};
        // Yahoo also exposes summary data under quoteSummary — try both shapes
    ...
    // --- Merge: Finnhub first, Yahoo fills any nulls ---
...
addBtn.addEventListener('click', () => {
    ...
    // Guard: don't add if data hasn't loaded yet
